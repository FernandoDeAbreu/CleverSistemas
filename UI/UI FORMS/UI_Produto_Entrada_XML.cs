...
{
    public partial class UI_Produto_Entrada_XML : Sistema.UI.UI_Modelo
    {
        public UI_Produto_Entrada_XML()
        {
            InitializeComponent();
        }
        ...
        DTO_Produto_Fornecedor Produto_Fornecedor;
        #endregion
                    ...
                    else
                    {
                        ...
                        switch (Parametro_Venda.Produto_PrecoVenda)
                        {
                            ...
                            case Composicao_PrecoVenda.Preco_Compra:
                                ...
                                //if (Parametro_Cadastro.EntradaProduto == 1) //ATUALIZA MARGEM
                                //{
                                //    txt_Margem.Text = util_dados.ConfigNumDecimal(util_dados.CalculaMargem(Produto_Item.ValorCompra, Convert.ToDouble(DTProduto.Rows[0]["ValorVenda"]) - Produto_Item.ValorIPI - Produto_Item.ValorST), 2);
                                //    txt_ValorVenda.Text = util_dados.ConfigNumDecimal(Convert.ToDouble(DTProduto.Rows[0]["ValorVenda"]), 2);
                                //}
                                //if (Parametro_Cadastro.EntradaProduto == 2) //ATUALIZA VALOR DE VENDA
                                //{
                                //    txt_Margem.Text = util_dados.ConfigNumDecimal(Convert.ToDouble(DTProduto.Rows[0]["MargemVenda"]), 2);
                                //    txt_ValorVenda.Text = util_dados.ConfigNumDecimal(util_dados.CalculaValor(Produto_Item.ValorCompra, Convert.ToDouble(DTProduto.Rows[0]["MargemVenda"])) + Produto_Item.ValorIPI + Produto_Item.ValorST, 2);
                                //}
                                break;
                        }
                    }
        ...
        private DataTable Busca_Produto_NFe(int ID_Fornecedor, string Codigo_Produto)
        {
            BLL_Produto = new BLL_Produto();
            Produto = new DTO_Produto();
            Produto.Fornecedor = new List<DTO_Produto_Fornecedor>();
            Produto_Fornecedor = new DTO_Produto_Fornecedor();

            Produto.ID_Empresa = Parametro_Empresa.ID_Empresa_Ativa;
            Produto_Fornecedor.ID_Fornecedor = ID_Fornecedor;
            Produto_Fornecedor.Codigo_Produto = Codigo_Produto;

            Produto.Fornecedor.Add(Produto_Fornecedor);

            return BLL_Produto.Busca_Produto_Fornecedor(Produto);
        ...
        private void Busca_Item(int _ID)
        {
            ...
            for (int i = 0; i <= _DT.Rows.Count - 1; i++)
            {
                ...
                Produto_Item.NFe_Descricao = _DT.Rows[i]["NFe_Descricao"].ToString();
                Produto_Item.NFe_CodigoProduto = _DT.Rows[i]["NFe_CodigoProduto"].ToString();
        ...
        private void Carrega_Item(List<DTO_Produto_Item> _lst_Produto)
        {
            ...
            for (int i = 0; i <= _lst_Produto.Count - 1; i++)
            {
                ...
                dg_Itens.Rows[i].Cells["col_NFe_CodigoProduto"].Value = _lst_Produto[i].NFe_CodigoProduto;
                dg_Itens.Rows[i].Cells["col_NFe_Descricao"].Value = _lst_Produto[i].NFe_Descricao;
                ...
                dg_Itens.Rows[i].Cells["col_MargemVenda"].Value = _lst_Produto[i].Margem;
                dg_Itens.Rows[i].Cells["col_ValorVenda"].Value = _lst_Produto[i].ValorVenda;
                ...
                if (_lst_Produto[i].ID_Produto == 0)
        ...
        private void Importa_XML(string _Arquivo)
        {
            try
            {
                ...
                DataTable _DT;
                DataTable _DT_Produto;

                lst_Produto = new List<DTO_Produto_Item>();

                xml_Arquivo = new XmlDocument();

                xml_Arquivo.Load(_Arquivo);

                string _str_xml = File.ReadAllText(_Arquivo);

                // GERA ESTRUTURA XML NFe               
                var xml_NFe = util_dados.LoadFromXMLString<NFe.ProcNFe.TNfeProc>(_str_xml);

                Chave = xml_NFe.NFe.infNFe.Id;
                Chave_NFe = xml_NFe.NFe.infNFe.Id;
                ...
                if (xml_NFe.NFe.infNFe.emit.ItemElementName.ToString() == "CNPJ")
                    Pessoa.CNPJ_CPF = util_dados.Conf_CPF_CNPJ(xml_NFe.NFe.infNFe.emit.Item.ToString(), Documento.CNPJ);
                else if (xml_NFe.NFe.infNFe.emit.ItemElementName.ToString() == "CPF")
                    Pessoa.CNPJ_CPF = util_dados.Conf_CPF_CNPJ(xml_NFe.NFe.infNFe.emit.Item.ToString(), Documento.CPF);
                ...
                if (_DT.Rows.Count == 0)
                {
                    DialogResult msgbox = MessageBox.Show(util_msg.msg_Fornecedor_NãoEncontrado, this.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2);
                    ...
                    BLL_Pessoa = new BLL_Pessoa();
                    Pessoa = new DTO_Pessoa();
                    Pessoa.Endereco = new List<DTO_Pessoa_Endereco>();
                    ...
                    Pessoa.Matricula = "";
                    Pessoa.Salario = 0;
                    Pessoa.CarteiraProfissional = "";
                    Pessoa.PIS = "";
                    Pessoa.MultiEmpresa = true;

                    if (xml_NFe.NFe.infNFe.emit.ItemElementName.ToString() == "CNPJ")
                    {
                        Pessoa.TipoDocumento = 1;
                        Pessoa.CNPJ_CPF = util_dados.Conf_CPF_CNPJ(xml_NFe.NFe.infNFe.emit.Item.ToString(), Documento.CNPJ);
                    }
                    else if (xml_NFe.NFe.infNFe.emit.ItemElementName.ToString() == "CPF")
                    {
                        Pessoa.TipoDocumento = 2;
                        Pessoa.CNPJ_CPF = util_dados.Conf_CPF_CNPJ(xml_NFe.NFe.infNFe.emit.Item.ToString(), Documento.CPF);
                    }

                    Pessoa.Nome_Razao = xml_NFe.NFe.infNFe.emit.xNome;
                    Pessoa.NomeFantasia = xml_NFe.NFe.infNFe.emit.xFant;
                    Pessoa.Contato = "";
                    Pessoa.IE_RG = xml_NFe.NFe.infNFe.emit.IE;
                    Pessoa.IM = "";
                    Pessoa.Cadastro = DateTime.Now;
                    Pessoa.Informacao = "";
                    Pessoa.Informacao_Venda = "";
                    Pessoa.Dia_Faturamento = "0";

                    Pessoa.ID_Grupo = 0;

                    Pessoa.Nascimento_Fundacao = DateTime.Now;
                    Pessoa.Ramo_Profissao = "";
                    Pessoa.Descricao1 = "";
                    Pessoa.Descricao2 = "";
                    Pessoa.Descricao3 = "";
                    Pessoa.Limite = 0;
                    Pessoa.DiaFaturamento = 0;
                    Pessoa.Situacao = true;
                    Pessoa.CEI = "";
                    Pessoa.ValorMensalidade = 0;
                    Pessoa.Conjuge = "";
                    Pessoa.CPF_Conjuge = "";
                    Pessoa.RG_Conjuge = "";
                    Pessoa.Profissao_Conjuge = "";

                    #region ENDEREÇO
                    Endereco = new DTO_Pessoa_Endereco();

                    Endereco.Principal = true;
                    Endereco.Tipo = Parametro_Cadastro.Endereco_Padrao;
                    Endereco.Logradouro = xml_NFe.NFe.infNFe.emit.enderEmit.xLgr;
                    Endereco.Numero = xml_NFe.NFe.infNFe.emit.enderEmit.nro;
                    Endereco.Complemento = xml_NFe.NFe.infNFe.emit.enderEmit.xCpl;
                    Endereco.Bairro = xml_NFe.NFe.infNFe.emit.enderEmit.xBairro;

                    BLL_Municipio = new BLL_Municipio();
                    Municipio = new DTO_Municipio();

                    Municipio.ID_Municipio = Convert.ToInt32(xml_NFe.NFe.infNFe.emit.enderEmit.cMun);

                    Endereco.ID_Municipio = Convert.ToInt32(BLL_Municipio.Busca_Municipio(Municipio).Rows[0]["ID"]);

                    Endereco.CEP = xml_NFe.NFe.infNFe.emit.enderEmit.CEP;
                    ...
                    Telefone.Principal = true;
                    Telefone.Tipo = Parametro_Cadastro.Telefone_Padrao;

                    string DDD = string.Empty;
                    string Fone = string.Empty;

                    if (xml_NFe.NFe.infNFe.emit.enderEmit.fone.Length >= 10 &&
                        xml_NFe.NFe.infNFe.emit.enderEmit.fone.Length <= 12)
                    {
                        if (xml_NFe.NFe.infNFe.emit.enderEmit.fone.Substring(0, 1) == "0")
                        {
                            DDD = xml_NFe.NFe.infNFe.emit.enderEmit.fone.Substring(1, 2);
                            Fone = xml_NFe.NFe.infNFe.emit.enderEmit.fone.Substring(3);
                            Fone = Fone.Substring(0, Fone.Length - 4) + "-" + Fone.Substring(Fone.Length - 4);
                        }
                        else if (xml_NFe.NFe.infNFe.emit.enderEmit.fone.Length == 10 ||
                                 xml_NFe.NFe.infNFe.emit.enderEmit.fone.Length == 11)
                        {
                            DDD = xml_NFe.NFe.infNFe.emit.enderEmit.fone.Substring(0, 2);
                            Fone = xml_NFe.NFe.infNFe.emit.enderEmit.fone.Substring(2);
                            Fone = Fone.Substring(0, Fone.Length - 4) + "-" + Fone.Substring(Fone.Length - 4);
                        }
                    }
                    else
                    {
                        DDD = "00";
                        Fone = xml_NFe.NFe.infNFe.emit.enderEmit.fone.PadLeft(9, '0').Substring(0, 9);
                    }
                    ...
                    if (obj != 0)
                    {
                        ...
                        cb_ID_Pessoa.SelectedValue = obj;
                    }
                    #endregion
                }
                else
                ...
                int _aux_Prod = xml_NFe.NFe.infNFe.det.Length;

                for (int i = 0; i <= _aux_Prod - 1; i++)
                {
                    Produto_Item = new DTO_Produto_Item();

                    Produto_Item.NFe_CodigoProduto = xml_NFe.NFe.infNFe.det[i].prod.cProd;
                    Produto_Item.NFe_Descricao = xml_NFe.NFe.infNFe.det[i].prod.xProd;
                    Produto_Item.Barra = xml_NFe.NFe.infNFe.det[i].prod.cEAN;
                    Produto_Item.NCM = xml_NFe.NFe.infNFe.det[i].prod.NCM;
                    Produto_Item.CEST = xml_NFe.NFe.infNFe.det[i].prod.CEST;
                    Produto_Item.NFe_Unidade = xml_NFe.NFe.infNFe.det[i].prod.uCom;
                    Produto_Item.Quantidade = Convert.ToDouble(xml_NFe.NFe.infNFe.det[i].prod.qCom.Replace(".", ","));
                    Produto_Item.ValorCompra = Convert.ToDouble(xml_NFe.NFe.infNFe.det[i].prod.vUnCom.Replace(".", ","));

                    Produto_Item.ValorST = 0;
                    Produto_Item.ValorIPI = 0;

                    #region VERIFICA VALOR IMPOSTOS (ST E IPI)
                    var imp = xml_NFe.NFe.infNFe.det.Select(x => x.imposto);
                    ...
                    foreach (var imposto in imp)
                    {
                        if (_aux_Imposto == i)
                        {
                            ...
                            for (int j = 0; j < (imposto.Items.Length); j++)
                            {
                                //string _aux = imposto.Items[j].GetType().ToString();
                                switch (imposto.Items[j].GetType().ToString())
                                {
                                    #region ICMS ST
                                    case "Sistema.NFe.ProcNFe.TNFeInfNFeDetImpostoICMS":
                                        object ICMS = (((NFe.ProcNFe.TNFeInfNFeDetImpostoICMS)(imposto.Items[j])).Item);

                                        switch (ICMS.GetType().ToString())
                                        {
                                            case "Sistema.NFe.ProcNFe.TNFeInfNFeDetImpostoICMSICMS10":
                                                Produto_Item.ValorST = util_dados.Verifica_Double(((NFe.ProcNFe.TNFeInfNFeDetImpostoICMSICMS10)ICMS).vICMSST.Replace(".", ","));
                                                break;

                                            case "Sistema.NFe.ProcNFe.TNFeInfNFeDetImpostoICMSICMS30":
                                                Produto_Item.ValorST = util_dados.Verifica_Double(((NFe.ProcNFe.TNFeInfNFeDetImpostoICMSICMS30)ICMS).vICMSST.Replace(".", ","));
                                                break;

                                            case "Sistema.NFe.ProcNFe.TNFeInfNFeDetImpostoICMSICMS70":
                                                Produto_Item.ValorST = util_dados.Verifica_Double(((NFe.ProcNFe.TNFeInfNFeDetImpostoICMSICMS70)ICMS).vICMSST.Replace(".", ","));
                                                break;

                                            case "Sistema.NFe.ProcNFe.TNFeInfNFeDetImpostoICMSICMS90":
                                                Produto_Item.ValorST = util_dados.Verifica_Double(((NFe.ProcNFe.TNFeInfNFeDetImpostoICMSICMS90)ICMS).vICMSST.Replace(".", ","));
                                                break;

                                            case "Sistema.NFe.ProcNFe.TNFeInfNFeDetImpostoICMSICMSSN201":
                                                Produto_Item.ValorST = util_dados.Verifica_Double(((NFe.ProcNFe.TNFeInfNFeDetImpostoICMSICMSSN201)ICMS).vICMSST.Replace(".", ","));
                                                break;

                                            case "Sistema.NFe.ProcNFe.TNFeInfNFeDetImpostoICMSICMSSN202":
                                                Produto_Item.ValorST = util_dados.Verifica_Double(((NFe.ProcNFe.TNFeInfNFeDetImpostoICMSICMSSN202)ICMS).vICMSST.Replace(".", ","));
                                                break;

                                            case "Sistema.NFe.ProcNFe.TNFeInfNFeDetImpostoICMSICMSSN900":
                                                Produto_Item.ValorST = util_dados.Verifica_Double(((NFe.ProcNFe.TNFeInfNFeDetImpostoICMSICMSSN900)ICMS).vICMSST.Replace(".", ","));
                                                break;
                                        }
                                        break;
                                    ...
                                    #region IPI
                                    case "Sistema.NFe.ProcNFe.TIpi":
                                        object IPI = (((NFe.ProcNFe.TIpi)(imposto.Items[j])).Item);

                                        if (IPI.GetType().ToString() == "Sistema.NFe.ProcNFe.TIpiIPITrib")
                                            switch (((NFe.ProcNFe.TIpiIPITrib)IPI).CST.ToString())
                                            {
                                                case "Item00":
                                                case "Item49":
                                                case "Item50":
                                                case "Item99":
                                                    Produto_Item.ValorIPI = util_dados.Verifica_Double(((NFe.ProcNFe.TIpiIPITrib)IPI).vIPI.Replace(".", ","));
                                                    break;
                                            }
                                        break;
                                        #endregion
                            ...
                            if (Produto_Item.ValorST > 0)
                                Produto_Item.ValorST = Produto_Item.ValorST / Produto_Item.Quantidade;

                            if (Produto_Item.ValorIPI > 0)
                                Produto_Item.ValorIPI = Produto_Item.ValorIPI / Produto_Item.Quantidade;
                    ...
                    double CustoFinal = 0;
                    CustoFinal += Produto_Item.ValorCompra;
                    CustoFinal += Produto_Item.ValorST;
                    CustoFinal += Produto_Item.ValorIPI;

                    #region CONSULTA PRODUTO
                    _DT_Produto = new DataTable();
                    _DT_Produto = Busca_Produto_NFe(Convert.ToInt32(cb_ID_Pessoa.SelectedValue), xml_NFe.NFe.infNFe.det[i].prod.cProd);

                    if (_DT_Produto.Rows.Count == 1)
                    {
                        Produto_Item.ID_Produto = Convert.ToInt32(_DT_Produto.Rows[0]["ID"]);
                        Produto_Item.Descricao_Produto = _DT_Produto.Rows[0]["Descricao"].ToString();
                        Produto_Item.ID_Grade = Convert.ToInt32(_DT_Produto.Rows[0]["ID_Grade"]);
                        Produto_Item.ID_Fornecedor = Convert.ToInt32(cb_ID_Pessoa.SelectedValue);
                        ...
                        switch (Parametro_Venda.Produto_PrecoVenda)
                        {
                            case Composicao_PrecoVenda.Custo_Final:
                                if (Parametro_Cadastro.EntradaProduto == 1) //ATUALIZA MARGEM
                                {
                                    Produto_Item.Margem = util_dados.CalculaMargem(CustoFinal, Convert.ToDouble(_DT_Produto.Rows[0]["ValorVenda"]));
                                ...
                                if (Parametro_Cadastro.EntradaProduto == 2) //ATUALIZA VALOR DE VENDA
                                {
                                    ...
                                    Produto_Item.ValorVenda = util_dados.CalculaValor(CustoFinal, Convert.ToDouble(_DT_Produto.Rows[0]["MargemVenda"]));
                                }
                                break;
                            ...
                            case Composicao_PrecoVenda.Preco_Compra:
                                if (Parametro_Cadastro.EntradaProduto == 1) //ATUALIZA MARGEM
                                {
                                    Produto_Item.Margem = util_dados.CalculaMargem(Produto_Item.ValorCompra, Convert.ToDouble(_DT_Produto.Rows[0]["ValorVenda"]) - Produto_Item.ValorIPI - Produto_Item.ValorST);
                                    Produto_Item.ValorVenda = Convert.ToDouble(_DT_Produto.Rows[0]["ValorVenda"]);
                                }
                                if (Parametro_Cadastro.EntradaProduto == 2) //ATUALIZA VALOR DE VENDA
                                {
                                    Produto_Item.Margem = Convert.ToDouble(_DT_Produto.Rows[0]["MargemVenda"]);
                                    Produto_Item.ValorVenda = util_dados.CalculaValor(Produto_Item.ValorCompra, Convert.ToDouble(_DT_Produto.Rows[0]["MargemVenda"])) + Produto_Item.ValorIPI + Produto_Item.ValorST;
                                }
                                break;
                        }
                    }
                ...
                txt_Documento.Text = "NF-e: " + xml_NFe.NFe.infNFe.ide.nNF.TrimStart('0') + " Chave(" + xml_NFe.NFe.infNFe.Id + ")";
            }
            catch (Exception ex)
            {
        ...
        public override void Pesquisa()
        {
            ...
            Produto_Entrada.Tipo_Entrada = 3;
        ...
        public override void Gravar()
        {
            try
            {
                ...
                Produto_Entrada.Entrega = DateTime.Now;
                Produto_Entrada.ID_Usuario = Parametro_Usuario.ID_Usuario_Ativo;

                Produto_Entrada.Entrada_XML_NFe = true;
                ...
                if (obj > 0)
                {
                    ...
                    string _Periodo = util_dados.Config_Data(Convert.ToDateTime(DateTime.Now), 9).ToString();

                    string _xml = Parametro_NFe_NFC_SAT.Caminho_NFe + util_Param.XML_Entrada + _Periodo + @"\" + Chave_NFe + NFe_extxml.Nfe_Entrada;
                    string _xml2 = Parametro_Sistema.Caminho_Sistema + @"\Temp\" + Chave_NFe + NFe_extxml.Nfe_Entrada;

                    if (!Directory.Exists(Parametro_Sistema.Caminho_Sistema + @"\Temp"))
                        Directory.CreateDirectory(Parametro_Sistema.Caminho_Sistema + @"\Temp");

                    XmlTextWriter xWriter = new XmlTextWriter(@_xml2, Encoding.UTF8);
                    xWriter.Formatting = Formatting.None;
                    xml_Arquivo.Save(xWriter);

                    xWriter.Close();

                    if (!Directory.Exists(Parametro_NFe_NFC_SAT.Caminho_NFe + util_Param.XML_Entrada + _Periodo))
                        Directory.CreateDirectory(Parametro_NFe_NFC_SAT.Caminho_NFe + util_Param.XML_Entrada + _Periodo);

                    if (!File.Exists(_xml))
                        File.Move(_xml2, _xml);
                }
        ...
        private void bt_ImportaXML_Click(object sender, EventArgs e)
        {
            Pesquisa_Arquivo.Filter = "Arquivos XML|*.xml";
            Pesquisa_Arquivo.ShowDialog();

            if (Pesquisa_Arquivo.FileName.ToString() != string.Empty)
                Importa_XML(Pesquisa_Arquivo.FileName);
        }
        #endregion
